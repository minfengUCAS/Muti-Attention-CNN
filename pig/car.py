...
model_weights ='model/car_part.caffemodel'
model_def ='deploy/car_part_deploy.prototxt'
p_net = caffe.Net(model_def,model_weights,caffe.TEST)

model_weights ='model/car_class_iter_0.caffemodel'
model_def ='deploy/car_cls_deploy.prototxt'
c_net = caffe.Net(model_def,model_weights,caffe.TEST)

test_list = open('car_data/test_list.txt').readlines()
accuracy = 0
# feature = np.zeros((ls,2560))
for i in range(ls):	
	print i
	if i>-1:
		img = cv2.imread('car_data/' + test_list[i].split(' ')[0])
		if img.ndim<3:
		...
		c_net.blobs['ori_data'].data[...] = crop_lit_centor(img)
...
# np.savetxt('\\\\msralab\\ProjectData\\MSMDATA-3\\jianf\\share\\for.heliang\\code_for_release\\iccv_2017\\data\\car_f_test.txt',feature)
